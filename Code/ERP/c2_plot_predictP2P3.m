...
for i=1:sizT(2);
    for j=1:sizX(2);
        indx1 = find(clERP(:,4) == t(i));
        indx2 = find(clERP(:,5) == x(j)); 
        indx12 = intersect(indx1,indx2);
        
        indx0 = find(clERP(:,3) == isub);
        indx = intersect(indx0,indx12);
        
        mErpP2(i,j,isub) = mean(clERP(indx,1));
        mErpP3(i,j,isub) = mean(clERP(indx,2));
